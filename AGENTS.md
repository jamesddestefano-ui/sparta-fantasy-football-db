...
58. Maintain Pulse aggregate learning in `intelligence/sparta_pulse_learning.json`: pulses_considered, pulses_used, pulses_rejected, actions_changed_by_pulse, successful_actions_changed_by_pulse, unsuccessful_actions_changed_by_pulse, average_information_lead_time, highly_useful_pulses, misleading_pulses, too_late_pulses, pulse_category_performance, and pulse_source_performance.
59. Every weekly Sparta learning review must include an `NFL PULSE PERFORMANCE` section covering the metrics in rule 58 plus notable examples and unresolved Pulse-linked decisions.
60. Never promote a Pulse-derived signal into a permanent Sparta rule from one ordinary result. Require repeated evidence unless the event exposes a clear structural defect.
61. Pulse lineage is Sparta-specific. Never import or learn from Mongo ownership/FAAB/waiver/roster/decision state, DFS lineup state, or props betting state. Shared NFL facts may be referenced through NFL Pulse only; system-specific state may not cross boundaries.
